...
import { createOrder, deleteOrder, getMyOrders, getMyOrdersPage, getOrder, getOrders, getOrdersPage, getOrderStatusCounts, updateOrder, updateOrderStatus, type OrderFilters, type OrderInput, type OrderStatus } from "@/api/orderApi";
...
export function useOrdersPage(filters?: OrderFilters, pagination?: PageParams, enabled = true, refetchInterval?: number) { return useQuery({ queryKey: [...key, "page", filters, pagination], queryFn: () => getOrdersPage(filters, pagination), enabled, refetchInterval }); }
export function useMyOrdersPage(pagination?: PageParams, enabled = true, refetchInterval?: number) { return useQuery({ queryKey: [...key, "my", "page", pagination], queryFn: () => getMyOrdersPage(pagination), enabled, refetchInterval }); }
export function useOrderStatusCounts(enabled = true, refetchInterval?: number) { return useQuery({ queryKey: [...key, "status-counts"], queryFn: getOrderStatusCounts, enabled, refetchInterval }); }
