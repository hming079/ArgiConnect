...
class EventConfig {
    ...
    @Bean
    org.springframework.amqp.core.Queue orderDlq() {
        return QueueBuilder.durable("logistics.order-events.dlq").build();
    }

    @Bean
    org.springframework.amqp.core.Queue orderQueue() {
        return QueueBuilder.durable("logistics.order-events").deadLetterExchange("")
                .deadLetterRoutingKey("logistics.order-events.dlq").build();
    }

    @Bean
    Binding orderBinding(org.springframework.amqp.core.Queue orderQueue, TopicExchange events) {
        return BindingBuilder.bind(orderQueue).to(events).with("order.confirmed");
    }
}
...
class Outbox {
    ...
    void add(String aggregate, String id, String type, String routing, Object payload) {
        try {
            ...
            Map<String, Object> v = Map.of("eventId", event, "eventType", type, "eventVersion", 1, "aggregateType",
                    aggregate, "aggregateId", id, "occurredAt", Instant.now(), "traceId", UUID.randomUUID(), "producer",
                    "logistics-service", "routingKey", routing, "payload", payload);
            db.sql("insert into outbox_events(id,aggregate_type,aggregate_id,event_type,event_version,payload,status,created_at)values(:id,:a,:g,:t,1,cast(:p as jsonb),'PENDING',now())")
                    .param("id", event).param("a", aggregate).param("g", id).param("t", type)
                    .param("p", json.writeValueAsString(v)).update();
        } catch (Exception e) {
