...
class InternalApiKeyFilter extends OncePerRequestFilter {
    ...
    InternalApiKeyFilter(@Value("${internal.api-key}") String k) {
    ...
    protected void doFilterInternal(HttpServletRequest r, HttpServletResponse p, FilterChain c)
        ...
        String value = r.getHeader("X-Internal-Api-Key");
        if (value == null || !MessageDigest.isEqual(key, value.getBytes(StandardCharsets.UTF_8))) {
            p.sendError(403, "Invalid internal API key");
            return;
        }
        c.doFilter(r, p);
    }
